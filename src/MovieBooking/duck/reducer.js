...
export const movieBookingReducer = ( state=initialState, action) => {
    switch (action.type){
        case CHAIR_BOOKING: {
            const chairBookingListClone = [...state.chairBookingList];
            const index = chairBookingListClone.findIndex(item=>item.soGhe === action.payload.soGhe);
            if(index !== -1){
                chairBookingListClone.splice(index,1)
            }else {
                chairBookingListClone.push(action.payload)
            }

            state.chairBookingList = chairBookingListClone;
            return {...state};
        }
        ...
        case PAY: {
            const danhSachSoGheDangChon = state.chairBookingList.map(item=>item.soGhe)
            console.log('danhSachSoGheDangChon',danhSachSoGheDangChon);

            const data = state.chairList.map(item=>{
                const dsGheNew = item.danhSachGhe.map(chair=>{
                    if(danhSachSoGheDangChon.includes(chair.soGhe)){
                        chair.daDat = true;
                    }
